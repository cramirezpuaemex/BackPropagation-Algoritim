...
from ResultadosClasificarNew import *
import decimal
import random
import math
...
class Clasificar():
	...
	def pathfilePL(self):
		...
		self.filename = askopenfilename(title="Ingresa el archivo que contiene los parametros libres obtenidos del entrenamiento Clas.") # show an "Open" dialog box and return the path to the selected file
		return(self.filename)
	def pathfileME(self):
		Tk().withdraw() # we don't want a full GUI, so keep the root window from appearing
		self.filename = askopenfilename(title="Ingresa el archivo que contiene la media y desviacion estandar obtenido de la normalizacion Clas.") # show an "Open" dialog box and return the path to the selected file
		return(self.filename)
	def pathfileAN(self):
		Tk().withdraw() # we don't want a full GUI, so keep the root window from appearing
		self.filename = askopenfilename(title="Ingresa el archivo que contiene los datos para realizar la clasificacion") # show an "Open" dialog box and return the path to the selected file
		return(self.filename)
	...
	def trabajar(self):
		...
		if respuesta == True:
			...
			self.app.destroy()
			print " vamos bien :)"
			inicia=ResultadosClasificarNew()
	...
	def validar(self):
		...
		if self.pathArchivoDE == None:
			...
			valido = False
			tkMessageBox.showinfo("Error", "Debe seleccionar un archivo de datos para clasificar")
		else:
			valido = True
	...
	def main(self):
		...
		self.app = Tk()
		self.app.title('Clasificacion (Datos que se desconoce la clase a la que pertenecen)')
		...
		boton.grid(column=2,row=2)
 		########################################################################################
				#######################################################################################
		etiquetaDA = Label(vp,text="Archivo de los datos para clasificar: ")
