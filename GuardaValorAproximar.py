...
class GuardaValorAproximar():
	filename = ""
	pathArchivoDE=None
	def pathfile(self):
		Tk().withdraw() # we don't want a full GUI, so keep the root window from appearing
		#tkMessageBox.showinfo("Abrir Archivo", "-> El archivo debe tener extencion .txt\n \n Selecciona el archivo normalizado sin clases para la opcion validacion cruzada Aproximar")

		self.filename = askopenfilename(title="Selecciona el archivo normalizado para validacion cruzada (Aprox).") # show an "Open" dialog box and return the path to the selected file
	...
	def validacionCruzada(self):
		...
		#print  "haber aqui debe de entrar"
		Limite=(len(self.X[-1])*80)/100
		#while contParticiones < self.numparticiones:
		cont=0
		while len(self.X[-1])!=0:
			#print  "longitud del archivo", len(self.X[-1])
		 	file=[]
		 	file1=[]
		 	#Limite=(len(self.X[-1])*80)/100
			...
			#print  "LImite", Limite
			if cont < Limite:
				#print  "entra al archivo entrenar"
