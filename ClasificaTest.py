...
class ClasificaTest():
	...
	def pathfileDE(self):
		...
		self.filename = askopenfilename(title="Seleccionar el archivo de parametros libres para clasificar. Debe tener extension .txt") # show an "Open" dialog box and return the path to the selected file
		return(self.filename)
	def pathfileDF(self):
		Tk().withdraw() # we don't want a full GUI, so keep the root window from appearing
		self.filename = askopenfilename(title="Selecciona el archivo normalizado de entrenamiento para clasificar. Debe tener extension .txt") # show an "Open" dialog box and return the path to the selected file
		return(self.filename)
	...
	def trabajar(self):
		...
		if respuesta == True:
			...
			inicio=clasifica()
			inicio.main(self.pathArchivoDE, self.pathArchivoDF)
	...
	def main(self):
		...
		self.app = Tk()
		self.app.title('Entrenamiento (Clasificar)')
